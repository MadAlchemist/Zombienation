...
public class Zombie8 extends Zombie {
   public Zombie8(EntityType<? extends Zombie> zombie, Level level) {
      super(zombie, level);
   }

   @Override
   protected boolean isSunSensitive() { return ConfigurationHandler.GENERAL.burnAtDay.get(); }
   protected SoundEvent getAmbientSound() { return SoundsRegistry.ZOMBIE8_AMBIENT; }
   ...
   public static void onDeath(LivingDeathEvent death) {
      if(death.getEntityLiving() instanceof Zombie8) {
         LootHelper.dropLoot(ConfigurationHandler.LOOT.zombie8_loot.get(),
